...
void update_ms_conf(int **state, MSTYPES *ms, STRINGS *str, float ener);
void print_ms(FILE *ms_out_fp, FILE *ms_crg_fp, MSTYPES *ms, STRINGS *str);
void MC_smp(int n, STRINGS *str, MSTYPES *ms); 
...
{  int i, j, k, counter, k_run;
    ...
    if (env.ms_out) {
        ...
        if (!ms_spe_lst.n) {
            ...
            remove("ms_out"); remove("ms_crg");
            ms_out_fp = fopen("ms_out", "a");
            ms_crg_fp = fopen("ms_crg", "a"); 
        }
    }
...
//        fprintf(ms_out_fp, "pH = %6.2f Eh = %6.2f:\n", ph, eh);
        fprintf(ms_crg_fp, "Titration (%dth) at pH = %6.2f Eh = %6.2f:\n", i+1, ph, eh);
        ...
        if (enumerate(i) == -1) {
            ...
            */
            ...
            for (j=0; j<env.monte_runs; j++) {
                ...
                if (N_smp) {
                    if (env.ms_out) MC_smp(N_smp, &ms_spe_lst, &ms);
                    else MC(N_smp);
                }
            ...
            time_test=0; 
            print_ms(ms_out_fp, ms_crg_fp, &ms, &ms_spe_lst); 
            free(ms.ms_type); free(ms.crg_type);
            memset(&ms, 0, sizeof(MSTYPES));
...
{
    FILE *ms_gold_fp;
    char line[MAXCHAR_LINE], sbuff[MAXCHAR_LINE];
    ATOM atom;
    int i_spe;

    if ((ms_gold_fp=fopen("ms_gold", "r"))) {
        printf("   loading file ms_gold\n"); fflush(stdout);
    ...
    for (i=0; i<cycles; i++) {
        ...
        */
        ...
        while (iters) {
            ...
            time_t1 = time(NULL);
            update_ms_conf(&state, ms, str, E_state); 
            time_test += time(NULL) - time_t1;
            /* count this state energy */
            H_average += E_state/n_total;
...
inline void update_ms_conf(int **state, MSTYPES *ms, STRINGS *str, float ener)
{
    int *ms_state, *crg_state;
    int i_spe, i_free, i_fix, ic, i_type;
    int found;

    ms_state = (int *) calloc(str->n, sizeof(int));
    crg_state = (int *) calloc(str->n, sizeof(int));

    for (i_spe=0; i_spe<str->n; i_spe++) {
        found = 0;
        for (i_free=0; i_free<n_free; i_free++) {
            if (!strncmp(conflist.conf[(*state)[i_free]].uniqID+5, str->strings[i_spe]+3, 5)) {
                found = 1;
                ms_state[i_spe]  = (*state)[i_free];
                crg_state[i_spe] = conflist.conf[(*state)[i_free]].H - conflist.conf[(*state)[i_free]].e;
                break;
            }
        }
        if (!found) {
            for (i_fix=0; i_fix<n_fixed; i_fix++) {
                if (!strncmp(conflist.conf[fixed_res[i_fix].conf[0]].uniqID+5, str->strings[i_spe]+3, 5)) {
                    for (ic=0; ic<fixed_res[i_fix].n; ic++) {
                        ...
                        if (conflist.conf[fixed_res[i_fix].conf[ic]].occ > 0.99) { 
                            ms_state[i_spe] = fixed_res[i_fix].conf[ic];
                            crg_state[i_spe] = conflist.conf[fixed_res[i_fix].conf[ic]].H 
                                             - conflist.conf[fixed_res[i_fix].conf[ic]].e;
                            found = 1;
                            break;
                        }
                    }
    ...
    /* update ms_type */
   for (i_type=ms->n_ms-1; i_type>=0; i_type--) {
        for (i_spe=0; i_spe<str->n; i_spe++) {
            if (ms_state[i_spe] != ms->ms_type[i_type].ic[i_spe]) break;
        }
        if (i_spe == str->n) {      // found
            ms->ms_type[i_type].occ ++;
            ms->ms_type[i_type].H_sum  += ener;
            ms->ms_type[i_type].H_2sum += ener * ener;
            break;
        }
    }
    if (i_type == -1) {      // not found
         ms->n_ms++;
         ms->ms_type   = (MSTYPE *) realloc(ms->ms_type, ms->n_ms * sizeof(MSTYPE));
         memset(&ms->ms_type[ms->n_ms-1], 0, sizeof(MSTYPE));
         ms->ms_type[ms->n_ms-1].ic = (int *) calloc(str->n, sizeof(int));
       
         for (i_spe=0; i_spe<str->n; i_spe++) {
             ms->ms_type[ms->n_ms-1].ic[i_spe] = ms_state[i_spe];
             ms->ms_type[ms->n_ms-1].e += conflist.conf[ms_state[i_spe]].e;
             ms->ms_type[ms->n_ms-1].H += conflist.conf[ms_state[i_spe]].H;
         }
         ms->ms_type[ms->n_ms-1].crg = ms->ms_type[ms->n_ms-1].H - ms->ms_type[ms->n_ms-1].e; 
         ms->ms_type[ms->n_ms-1].occ  = 1;
         ms->ms_type[ms->n_ms-1].H_sum  = ener;
         ms->ms_type[ms->n_ms-1].H_2sum = ener * ener;
    } 

    /* update crg_type */
    for (i_type=ms->n_crg-1; i_type>=0; i_type--) {
        for (i_spe=0; i_spe<str->n; i_spe++) {
            if (crg_state[i_spe] != ms->crg_type[i_type].icrg[i_spe]) break;
        }
        if (i_spe == str->n) {      // found
            ms->crg_type[i_type].occ ++;
            ms->crg_type[i_type].H_sum  += ener;
            ms->crg_type[i_type].H_2sum += ener * ener;
            break;
        }
    }
    if (i_type == -1) {      // not found
         ms->n_crg++;
         ms->crg_type   = (CRGTYPE *) realloc(ms->crg_type, ms->n_crg * sizeof(CRGTYPE));
         memset(&ms->crg_type[ms->n_crg-1], 0, sizeof(CRGTYPE));
         ms->crg_type[ms->n_crg-1].icrg = (int *) calloc(str->n, sizeof(int));
       
         for (i_spe=0; i_spe<str->n; i_spe++) {
             ms->crg_type[ms->n_crg-1].icrg[i_spe] = crg_state[i_spe];
             ms->crg_type[ms->n_crg-1].e += conflist.conf[ms_state[i_spe]].e;
             ms->crg_type[ms->n_crg-1].H += conflist.conf[ms_state[i_spe]].H;
         }
         ms->crg_type[ms->n_crg-1].crg = ms->crg_type[ms->n_crg-1].H - ms->crg_type[ms->n_crg-1].e; 
         ms->crg_type[ms->n_crg-1].occ  = 1;
         ms->crg_type[ms->n_crg-1].H_sum  = ener;
         ms->crg_type[ms->n_crg-1].H_2sum = ener * ener;
    } 
}       
...
/*    for (i_spe=0; i_spe<str->n; i_spe++) {
        fprintf(ms_out_fp, "%s   \t", str->strings[i_spe]);
    }
*/
    for (i_spe=0; i_spe<str->n; i_spe++) {
        fprintf(ms_crg_fp, "%s ", str->strings[i_spe]);
    }
//    fprintf(ms_out_fp, "%4c%4c%4s\t%s\t%s\t%s\n", 'H', 'e', "crg", "H_avg", "H_std", "occ");
    fprintf(ms_crg_fp, "%4c%4c%4s%8s%7s%7s\n", 'H', 'e', "crg", "H_avg", "H_std", "occ");

/*
    for (i_type=0; i_type<ms->n_ms; i_type++) {
        n_occ += ms->ms_type[i_type].occ;
    }
*/

    /* print for the file ms_out */
/*    for (i_type=0; i_type<ms->n_ms; i_type++) {
        for (i_spe=0; i_spe<str->n; i_spe++) {
            fprintf(ms_out_fp, "%s\t",  conflist.conf[ms->ms_type[i_type].ic[i_spe]].uniqID);
        }

        avg = ms->ms_type[i_type].H_sum / (double) ms->ms_type[i_type].occ;
        if (ms->ms_type[i_type].occ == 1) st = 0.0;
        else if (((ms->ms_type[i_type].H_2sum - avg * avg * ms->ms_type[i_type].occ)/(double) (ms->ms_type[i_type].occ
 - 1)) < 0.0) st = 0.0;  
        else {
            st  = (double) sqrt((double) (ms->ms_type[i_type].H_2sum - avg * avg * ms->ms_type[i_type].occ)/(double) (ms->ms_type[i_type].occ - 1));
               
        }
        
        fprintf(ms_out_fp, "%4d%4d%4d%10.2lf%6.2lf%8.2f%%\n", ms->ms_type[i_type].H, 
                                        ms->ms_type[i_type].e,
                                        ms->ms_type[i_type].crg,
                                        avg,
                                        st,
                                        ms->ms_type[i_type].occ * 100 / (float) n_occ);
    }
*/
    /* print for the file ms_crg */
    avg = 0.0; st = 0.0; n_occ = 0;
    for (i_type=0; i_type<ms->n_crg; i_type++) {
        n_occ += ms->crg_type[i_type].occ;
        printf("occ is %d\n", ms->crg_type[i_type].occ);
    }

    for (i_type=0; i_type<ms->n_crg; i_type++) {
        for (i_spe=0; i_spe<str->n; i_spe++) {
            fprintf(ms_crg_fp, "%8d ",  ms->crg_type[i_type].icrg[i_spe]);
        }
        avg = ms->crg_type[i_type].H_sum / (double) ms->crg_type[i_type].occ;
        if (ms->crg_type[i_type].occ == 1) st = 0.0;
        else if (((ms->crg_type[i_type].H_2sum - avg * avg * ms->crg_type[i_type].occ)/(double) (ms->crg_type[i_type].occ
 - 1)) < 0.0) st = 0.0;  
        else {
            st  = (double) sqrt((double) (ms->crg_type[i_type].H_2sum - avg * avg * ms->crg_type[i_type].occ)/(double) (ms->crg_type[i_type].occ - 1));
               
        }
        
        fprintf(ms_crg_fp, "%4d%4d%3d%10.2lf%6.2lf%8.2f%%\n", ms->crg_type[i_type].H, 
                                        ms->crg_type[i_type].e,
                                        ms->crg_type[i_type].crg,
                                        avg,
                                        st,
                                        ms->crg_type[i_type].occ * 100 / (float) n_occ);
    }
    fflush(ms_crg_fp);
}
